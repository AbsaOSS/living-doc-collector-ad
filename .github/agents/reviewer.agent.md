...
- Acceptance-criteria verification
  ...
  - Worked examples
    - Criterion "a disabled mode is skipped without running its collector" → open `main.run()`, confirm the `if ActionInputs.is_work_items_mode_enabled():` guard wraps `ADWorkItemsCollector(output_path).collect()`. A green `test_work_items_mode_disabled` that only asserts `assert_not_called()` is weaker than reading the guard.
    - Criterion "unparseable organizations JSON fails user-config validation" → confirm `ActionInputs.get_organizations()` raises `FetchOrganizationsException` on `json.JSONDecodeError`, and that `_validate()` increments `err_counter` in the `except FetchOrganizationsException` branch.
    - Criterion "the action output key is `output-path`" → confirm the literal `set_action_output("output-path", output_path)` call in `main.run()` and the matching `outputs.output-path` in `action.yml`.
- Quality
...
- Contract-sensitive outputs
  - Action output key `output-path` (set via `set_action_output("output-path", ...)`, exposed by `action.yml`).
  - Exit codes — `0` success, `1` any failure; no `2`–`5` taxonomy.
  - The `"Liv-Doc collector for Azure DevOps - ..."` step log strings asserted in `tests/unit/test_main.py`.
- High-risk areas
  - `INPUT_*` and `work-items-organizations` JSON parsing in `action_inputs.py`.
  - Azure DevOps REST API usage — `_call_profile_api` / `_call_org_api` in `action_inputs.py`: status-code handling, rate limits, error paths.
  - Filesystem writes under `output-path`.
  - Logging — must never leak the Azure DevOps PAT or `Authorization` headers; keep the whole collect path AI-free.
