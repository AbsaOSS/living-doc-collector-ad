...
- Spec locations
  - Prefer `README.md` for user-facing usage and examples, `DEVELOPER.md` for local-dev workflow, and `CONTRIBUTING.md` for contributor-facing rules.
  - Prefer a mode `README.md` (e.g. a future `work_items/README.md`) for mode-specific behavior once the mode is built; follow `.claude/rules/docs-lifecycle.md` when a prospective section ships.
- Contract-sensitive outputs
  ...
  - `INPUT_*` names in `utils/constants.py` and the `action.yml` input list — keep them in step.
- High-risk areas
  - Azure DevOps REST API usage in `action_inputs._validate()` — PAT validity, per-organization access, rate limiting, and non-200 handling.
  - GitHub Actions I/O — `INPUT_*` env var inputs and `output-path` writes.
  - `work-items-organizations` JSON parsing — malformed input and missing required fields.
