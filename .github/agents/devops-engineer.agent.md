...
- Runtime/toolchain targets
  - Python 3.10+ (ecosystem floor); CI matrix converges on `3.10` → `3.14` in the CI rollout task.
- Quality gates (use the `Makefile` targets — CI calls the same)
  - Full gate: `make qa`
  - Tests: `make test`
...
- Dependencies
  ...
  - Must pin GitHub Actions to a commit SHA, per `repo-conventions.md` §"Pinned actions".
- Contract-sensitive outputs
  - Action output key `output-path`; exit codes `0` / `1` only.
  - Must not change externally-visible action outputs or exit codes via CI changes.
