...
- Runtime/toolchain targets
  - Python 3.10+ (the ecosystem floor; the published action image may run a newer interpreter).
  - Must not add a 3.11+-only stdlib import or syntax without a `sys.version_info` fallback; `X | Y` unions and `match` / `case` are 3.10-native and stay.
- Logging conventions
...
- Quality gates (use the `Makefile` targets — CI runs the same)
  - Full gate: `make qa`
  - Tests: `make test`
...
- Contract-sensitive outputs
  - Action output key `output-path`; `INPUT_*` contract read only in `action_inputs.py` through `living_doc_utilities.github.utils.get_action_input`.
...
- AI-free principle
  - Must keep every collect → normalize → generate step deterministic — no LLM call in the collect path.
