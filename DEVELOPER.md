# Living Documentation Collector for Azure DevOps - for Developers
...
The published action image may run a newer interpreter.
...
The `Makefile` is the single source of truth for quality checks — the lint/test workflow
is converged onto the same targets, so local runs and CI never drift. Run the full gate
...
Also make sure that the INPUT_ADO_TOKEN is configured in your environment variables.
```
# Essential environment variables for ADO Action functionality
export INPUT_ADO_TOKEN=$(printenv ADO_TOKEN)
export INPUT_WORK_ITEMS=true
export INPUT_VERBOSE_LOGGING=true

# Environment variables for 'work-items' mode functionality
export INPUT_WORK_ITEMS_ORGANIZATIONS='[
  {
    "organization-name": "your-ado-organization",
    "projects-name-filter": ["Project Alpha"]
  }
]'
```
...
For running the ADO action locally, incorporate these commands into the shell script and save it.
```
python3 main.py
```
...
Example:
```shell
pylint work_items/collector.py
``` 
